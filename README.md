...
聊天 tab 对接 Discourse Chat：私信与频道合并为一条 inbox、会话消息收发、已读回执与新建 DM。iOS 进入会话后是盖住底栏的全屏二级页（与贴文详情相同）；Android 会话是独立 Activity。协议面见 `docs/knowledge/api/15-chat.md`。
...
当前仓库通过根目录 `rust-toolchain.toml` 固定 Rust `1.97.0`，workspace 的 `rust-version` 与之对齐为 `1.97`。
