...
4. **AppStateRefresher 由 Rust 单点编排** — 规格要求 authState 变化后，第一批立即刷新核心数据，第二批延迟 1 秒刷新次要数据。当前实现以 Rust `AppStateRefresher` 为唯一编排器；平台只触发 `triggerAppStateRefresh(...)` 并消费批次 callback，不再各自维护平台版 refresher。
