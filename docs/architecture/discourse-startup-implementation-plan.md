...
- `native/ios-app/App/Views/Other/FireTabRoot.swift` — SwiftUI auth gate（将被重写）
...
4. **AppStateRefresher 分两批刷新** — 规格要求 authState 变化后，第一批立即刷新核心数据（currentUser、categories、topicList），第二批延迟 1 秒刷新次要数据（userSummary、notifications、tags、bookmarks 等）。两端平台各实现 `AppStateRefresher`。
...
- `refresh_all(&self, trigger: RefreshTrigger)`:
  - 第一批（立即执行）：
    - 刷新 currentUser（`/session/current.json` 或 `/u/{username}.json`，2 分钟冷却）
    - 刷新 categories（从 preloaded data 或 `/categories.json`）
    - 刷新 topicTrackingStateMeta
    - 刷新 topicTrackingState
    - 刷新当前 tab 的 topicList（`/latest.json` 等）
  - 第二批（延迟 1 秒，避免并发过多触发风控）：
    ...
    - 我的话题（`/topics/created-by/{username}.json`）
    - MessageBus 重新初始化
- 去抖：2 秒内重复调用直接跳过
- 通过 `StateObserver.on_app_state_refresh_needed()` 通知平台各批次完成
...
#### File: `native/ios-app/App/Views/Other/FireTabRoot.swift`

重写。从 SwiftUI auth gate 改为：

- 未初始化：显示 `FirePreheatGateViewController`（via `UIViewControllerRepresentable`）
- 已初始化未登录：显示 `FireOnboardingViewController`（UIKit）
- 已初始化已登录：显示 `FireMainTabViewController`（UIKit `UITabBarController`）
...
#### File: `native/android-app/.../startup/AppStateRefresher.kt`（新文件）

```kotlin
class AppStateRefresher(private val sessionStore: FireSessionStore) {
    private var lastRefreshTime = 0L
    private val debounceMs = 2000L

    suspend fun refreshAll(trigger: RefreshTriggerState) {
        val now = System.currentTimeMillis()
        if (now - lastRefreshTime < debounceMs) return
        lastRefreshTime = now
        // 委托给 Rust FFI triggerAppStateRefresh
        sessionStore.triggerAppStateRefresh(trigger)
    }
}
```
...
修改启动条件：只在 `PreloadedDataResult` 中 `currentUser != nil` 时启动（规格 Section 12）。

#### File: `native/android-app/.../session/FireCfClearanceService.kt`

修改启动条件：只在 `PreloadedDataResult` 中 `currentUser != null` 时启动。
...
- `native/ios-app/App/Views/Other/FireTabRoot.swift` — **重写**：PreheatGate → Onboarding/MainTab
