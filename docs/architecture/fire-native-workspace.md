...
6. Before any authenticated request, hosts that keep browser cookies outside `session.json` must re-inject that platform cookie batch into Rust. The browser cookie store remains host-owned and authoritative on both iOS and Android; shared Rust session updates may sync into native networking storage such as `HTTPCookieStorage`, but must not write cookies back into the WebView cookie store.
...
8. If the restored session is otherwise ready but the local snapshot still lacks CSRF, hosts can rely on the shared authenticated-write preflight to fetch CSRF lazily when auth cookies are available. The Rust write path does not send mutations without an authenticated cookie set or without a usable CSRF token after refresh; those cases surface `MissingLoginSession` or `MissingCsrfToken` instead. `BAD CSRF` still triggers one clear-refresh-retry cycle, but only after the refreshed snapshot has both auth cookies and a CSRF token.
9. Use `fetch_topic_list` (global, category-scoped, tag-scoped, or private-message mailbox variants via `TopicListQuery`), the shared Rust topic-detail APIs (`fetch_topic_screen`, retained feed endpoints, `fetch_topic_response_page`, `fetch_topic_posts`, and `fetch_topic_ai_summary`) for the authenticated topic read paths. As of 2026-06-03, iOS topic detail opens as an app-root full-screen route and is aligned with Android on the `fetch_topic_screen` request path for initial load plus refreshes, while `fetch_topic_response_page` continues to drive reply pagination and targeted `fetch_topic_posts` hydration continues to fill anchor/reply-context gaps inside a partial response session. The processed feed APIs remain available but are no longer the active iOS detail entry path; the AI summary path remains a separate optional read and treats normal 403/404 as no available summary while preserving Cloudflare challenge errors for host recovery.
10. If Rust returns `CloudflareChallenge` for an authenticated operation, keep the current session snapshot and recover through a host-owned WebView loading a browser HTML URL, not the blocked JSON endpoint. iOS preserves the existing `cf_clearance`; topic-detail recoveries open `https://linux.do/t/{slug}/{topicId}` when a slug is known and `https://linux.do/t/{topicId}` otherwise, while homepage/list/notification recoveries open the site root. The recovery WebView waits for login readiness and a new `cf_clearance` relative to the recovery baseline before syncing the browser cookie batch into Rust and retrying the blocked operation; host-side cookie resync should treat batches that lack authenticated `_t` + `_forum_session` cookies as non-authoritative so an interactive challenge cannot silently wipe a valid native login snapshot. iOS and Android now share the same directionality here: WebView/browser cookies only flow into Rust, never the other way around. Android keeps the challenge WebView visible after `cf_clearance` appears: topic detail embeds `https://linux.do/t/{topicId}` below the toolbar, while other surfaces open `https://linux.do/`; both paths sync the browser cookie batch into Rust so subsequent native reads can continue without closing the WebView.
