...
10. If Rust returns `CloudflareChallenge` for an authenticated operation, keep the current session snapshot, present the forum home page (base URL) in a host-owned auth WebView (which naturally triggers Cloudflare Turnstile challenges), re-sync the latest same-site browser cookie batch into Rust after verification, and then retry the blocked operation from the host.
