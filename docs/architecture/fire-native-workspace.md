...
9. Use `fetch_topic_list` (global, category-scoped, tag-scoped, or private-message mailbox variants via `TopicListQuery`), the Rust topic-detail screen/feed APIs (`fetch_topic_screen`, `load_topic_detail_feed`, `refresh_topic_detail_feed`, and `fetch_topic_response_page`), and `fetch_topic_ai_summary` for the authenticated topic read paths. iOS topic detail should prefer the processed feed APIs for initial load and refresh so body/reply refreshes, stale snapshot handling, and response-session cursors stay Rust-owned; the AI summary path remains a separate optional read and treats normal 403/404 as no available summary while preserving Cloudflare challenge errors for host recovery.
