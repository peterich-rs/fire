...
- `native/ios-app` and `native/android-app`
  ...
  - Android main browser now renders its session/status controls, filter chips, topic rows, load-more state, and selected-topic summary as one `RecyclerView` + `ListAdapter` + `DiffUtil` screen rather than a `ScrollView` with manual row insertion
  ...
  - Android notification rows now render through `RecyclerView` + `ListAdapter` + `DiffUtil`, so single-read and mark-all-read changes update the affected rows without rebuilding the whole list
  ...
  - Android search results now render through `RecyclerView` + `ListAdapter` + `DiffUtil`, so grouped topic/post/user results can update by diff rather than rebuilding a scroll container
  - Android diagnostics and log viewers now render their trace/file lists through `RecyclerView` + `ListAdapter` + `DiffUtil`, keeping request browsing and log browsing low-churn for future crash/jank debugging work
  ...
  - Android topic detail now renders its post timeline with `RecyclerView` + `ListAdapter` + `DiffUtil`, so AI summary insertion and floor-level scroll targets can update the list without rebuilding the whole screen
...
8. If the restored session is otherwise ready but the local snapshot still lacks CSRF, do not require an eager refresh before the host can surface authenticated read state. Hosts may still call `refresh_csrf_token_if_needed` explicitly when a surface truly needs a fresh token, but iOS now leaves the normal repair path to authenticated-write preflight instead of `restoreColdStartSession()` or login completion.
...
10. If Rust returns `CloudflareChallenge` for an authenticated operation, keep the current session snapshot, delete stale `cf_clearance` values from the host WebView cookie store, present a host-owned auth WebView at the LinuxDo login URL, wait for the login readiness gate (username, same-site auth cookies, reusable bootstrap), sync the latest same-site browser cookie batch into Rust, and then retry the blocked operation from the host.
