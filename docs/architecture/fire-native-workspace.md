...
10. If Rust returns `CloudflareChallenge` for an authenticated operation, keep the current session snapshot, delete stale `cf_clearance` values from the host WebView cookie store, present `/challenge` in a host-owned auth WebView, re-sync the latest same-site browser cookie batch into Rust after verification, and then retry the blocked operation from the host.
