...
- The shared layer correctly classified `403 not_logged_in` as `LoginRequired`, and at the time also treated `discourse-logged-out` responses as `LoginRequired`; the current rule no longer applies that header to ordinary 403 `invalid_access` or successful responses.
...
- The recovery flow:
  ...
  4. Read the post-resync epoch. If it advanced, the resync actually rotated auth cookies and the caller may retry the original read once. If it did not advance, the resync produced no new auth state and the caller falls through to `handleRecoverableSessionErrorIfNeeded`, which now reports the `LoginRequired` error without clearing local session state.
- Diagnostics breadcrumbs and `auth` host log lines record both the success and the no-op cases for support triage.

### Read-site usage

- `FireHomeFeedStore.loadTopics` and `FireTopicDetailStore.loadTopicDetail` call the new helper before falling back to the shared recoverable-error handler.
...
- A new `readPathLoginRecoveryAttemptedEpochs` set lives on `FireAppViewModel`; when a resync rotates the session into a new epoch, only the markers tied to that new epoch are kept.
...
- Do not retry more than once per session epoch on the read path: the call site falls back to reporting the original error after that, so we do not mask repeated server-side invalidation.
- Do not auto-present the login WebView from passive read errors; explicit user login and Cloudflare recovery remain the presentation paths.
...
- It keeps `LoginRequired` visible to the user without destroying the local session snapshot.
