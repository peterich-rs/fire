...
- `refactor/openwire-upgrade` 集成线上的基线维护与最终集成
...
- Clean integration line：
  - worktree：`/Users/zhangfan/develop/github.com/fire-worktrees/refactor-openwire-upgrade`
  - branch：`refactor/openwire-upgrade`
  - 角色：维护 green baseline、验证 submodule 状态、承接最终集成
- Single feature line：
...
| Stage 3 | 待执行 | history / drafts inbox / social graph / invite / vote-poll / edit |
...
- `docs/architecture/fire-native-workspace.md` 明确 clean worktree 流程
...
Stage 3 不再新开 feature worktree，直接继续在 `feature/ios-beta` 上按子阶段线性推进。
...
2. 当前 clean baseline 是否仍然以 `refactor/openwire-upgrade` 为准
...
- 当前长期维护策略已经从“每阶段一个 feature worktree”收敛为：
  - 一个 clean integration line
  - 一个长期 feature line `feature/ios-beta`
- Stage 0、Stage 1、Stage 2 已完成。
- 下一步默认从 Stage 3A 开始，继续补 history / drafts inbox / social / vote / edit。
