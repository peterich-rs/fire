...
Status update (2026-05-31, fallback cleanup): topic-detail post rows now have one authoritative rendering path. The old `FireTopicDetailCollectionView` SwiftUI/ListKit surface, `FirePostRow`, and the runtime hosted post fallback were removed. Header/category and AI summary may still use SwiftUI-hosted cells as sparse screen chrome; stats, topic vote, body-state, replies header, reply-footer, notice, original post, and reply rows use native runtime cells. Post/reply layout misses, polls, rich text, and missing render data should be fixed in the native cell path rather than routed through a SwiftUI post-row fallback.
