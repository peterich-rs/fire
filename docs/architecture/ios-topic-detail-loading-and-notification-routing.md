...
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` `publishVisiblePostNumbersIfChanged()`, `loadMoreIfNeeded()`, and Texture range/display callbacks -- visible-row, scroll-metric, and batch-fetch callbacks used to trigger response-page preload and reading-time timing.
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` `handlePendingScrollTargetIfNeeded()` plus `FireTopicDetailRuntimeConfiguration.scrollItem(for:)` -- scroll retry path; the request stays pending until the target runtime item exists in the Texture collection snapshot.
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` plus `native/ios-app/App/ListKit/TopicDetail/FirePostCellNode.swift` -- native post/reply row rendering and viewport reporting for long threads.
