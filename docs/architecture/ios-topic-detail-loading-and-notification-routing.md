...
Status update (2026-05-31): the active iOS topic-detail runtime now treats first-level replies to the original post as the main reading stream and flattens nested reply chrome in the feed. For each first-level reply, inline secondary replies are limited to at most two selected by local quality signals from the loaded rows: accepted-answer state, like count, reaction totals, reply count, and stable floor order as the tie-break. Remaining already loaded nested replies expand inline when tapping "查看更多 ... 条回复"; the post-reply bottom sheet remains available for recursive reply-context loading and floor jumps. Long reply bodies are collapsed in the runtime cells with an "展开" affordance; tapping it marks that post expanded in host state and re-renders the full body.
