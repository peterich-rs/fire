...
Status update (2026-05-24): post-row reply swipe is now owned inside `FirePostRow` instead of wrapping rows only at the collection call site. `FireTopicReplySwipePolicy` keeps the arbitration testable: it reserves leading back navigation and trailing row controls, requires rightward horizontal intent, suppresses active rich-text selection, excludes row-local child frames for links/text, reply-target buttons, menus, images, polls, and horizontal reaction scrollers, and commits only when the final drag still satisfies the reply threshold.
...
- `native/ios-app/App/FireTopicDetailView.swift` `FirePostRow`, `FireSwipeToReplyContainer`, and `FireTopicReplySwipePolicy` -- current reply-swipe arbitration; the row marks interactive child frames and rich-text selection state so reply swipe does not steal text/link/control gestures.
- `native/ios-app/App/FireRichTextRenderer.swift` `FireRichTextView` -- reports active `UITextView` selection to the row-level swipe container while keeping links routed through the existing native link callback.
