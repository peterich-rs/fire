...
Status update (2026-05-31, fallback cleanup): topic-detail post rows now have one authoritative rendering path: `FireTopicDetailListViewController` binds original posts and replies to `FirePostCollectionViewCell`. The old `FireTopicDetailCollectionView` SwiftUI/ListKit surface, `FirePostRow`, and the runtime hosted post fallback were removed. Header/category, AI summary, topic vote, body-state, reply-footer, and notice rows may still use SwiftUI-hosted cells as screen chrome, but post/reply layout misses, polls, rich text, and missing render data should be fixed in the native cell path rather than routed through a SwiftUI post-row fallback.

Status update (2026-05-31, runtime smoothness): the app now points `native/ios-app/project.yml` at an explicit `Configs/Fire-Info.plist` template that carries `CADisableMinimumFrameDurationOnPhone`, so iPhone builds opt into ProMotion instead of depending on generated-plist build settings. The topic-detail bottleneck was still main-thread budget rather than an explicit 60 fps cap. The UIKit runtime no longer performs first-time rich-text height measurement synchronously from `IGListKit` sizing for native post rows. `FireTopicDetailListViewController` now prewarms native row layouts through the existing `FirePostLayoutManager` background queue, uses cheap estimated row layouts until exact TextKit measurements publish, and applies those published layouts with no-animation collection invalidation plus visible-cell rebinds. Poll-bearing original posts and replies now stay on the native path as well: poll option text is prepared into lightweight render models, poll heights are part of the background layout result, and `FirePostCollectionViewCell` renders poll cards with UIKit controls instead of falling back to SwiftUI `UIHostingController.sizeThatFits`. Native cells also avoid redoing stable avatar monogram work, image node trees, and reaction capsules when their signatures have not changed; working-range prefetch warms the row layout, avatar, and the first three cooked-image attachments instead of only the first inline image.
...
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` `publishVisiblePostNumbersIfChanged()`, `loadMoreIfNeeded()`, and IGListKit working-range callbacks -- visible-row, scroll-metric, and prefetch callbacks used to trigger response-page preload and reading-time timing.
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` `handlePendingScrollTargetIfNeeded()` plus `FireTopicDetailRuntimeConfiguration.scrollItem(for:)` -- scroll retry path; the request stays pending until the target runtime item exists in the IGListKit snapshot.
- `native/ios-app/App/TopicDetailRuntime/FireTopicDetailListViewController.swift` plus `native/ios-app/App/ListKit/TopicDetail/FirePostCollectionViewCell.swift` -- native post/reply row rendering and viewport reporting for long threads.
