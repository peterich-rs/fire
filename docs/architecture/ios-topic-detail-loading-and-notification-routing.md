...
Status update (2026-06-03): the redesign execution has moved the active topic
route onto `native/ios-app/App/TopicDetail/FireTopicDetailViewController`
through `FireTopicDetailControllerHost`. `App/TopicDetail/Feed/` now owns the
active Texture collection runtime, and the legacy SwiftUI page owner plus
duplicate `App/TopicDetailRuntime/` controller path have been removed from the
repository.

Status update (2026-06-03, later): topic detail is now routed as an app-root
full-screen presentation from `FireTabRoot` through a real
`UINavigationController`, instead of a tab-local push that hides the tab bar.
That keeps the controller-owned back/share/navigation chrome visible on the
independent topic page. The active iOS detail read path is also aligned with
Android again: initial load plus refreshes now use `fetchTopicScreen`
(`header + body + response`), while the older batched reply-session machinery
remains retained in `FireTopicDetailStore` as the active pagination/hydration
...
Status update (2026-05-30, feed-cache branch): topic-detail initial load now enters Rust through `loadTopicDetailFeed`, which wraps the existing `fetchTopicScreen` network path and persists the processed feed snapshot in `fire-store` SQLite storage under the app workspace cache. iOS bridges that snapshot back into the current `TopicScreenState` render path and renders it through the controller-owned `App/TopicDetail/Feed/` Texture runtime. That runtime uses `ASCollectionNode` for topic-detail rendering, Nuke 12.8.0 for the shared platform image pipeline, and Texture 3.2.0 through the local `TextureCore` XCFramework package for native async row layout/rendering. It skips no-op runtime updates, deduplicates scroll-driven visible-post notifications while force-publishing visibility after collection updates, and keys row identity off store-prepared render signatures instead of main-thread `cooked` HTML hashes. Incremental reply pagination remains on `fetchTopicResponsePage` until the Rust feed API grows a page-append command with the same cursor semantics.

Status update (2026-05-31): the active iOS topic-detail runtime treats first-level replies to the original post as the main reading stream. Nested reply rows stay attached to their root reply instead of being promoted into the main stream: by default the root row exposes a "查看更多 ... 条回复" shortcut, and tapping it expands the already loaded child rows inline with their Rust-provided depth preserved. A pending deep-link or notification anchor may reveal the target child row plus its loaded ancestry so scrolling can complete without flattening the hierarchy. Long reply bodies are collapsed in the runtime cells with an inline `... 展开` truncation token owned by `ASTextNode`; tapping the token marks that post expanded in host state and re-renders the full body. Media and polls that belong to a collapsed reply stay hidden until expansion, and reply images are rendered at a reduced bounded size so comment media does not dominate the reading stream.

Status update (2026-05-31, fallback cleanup): topic-detail post rows now have one authoritative rendering path. The old `FireTopicDetailCollectionView` SwiftUI/ListKit surface, `FirePostRow`, and the runtime hosted post fallback were removed. At that point header/category and AI summary still used sparse hosted chrome cells; stats, topic vote, body-state, replies header, reply-footer, notice, original post, and reply rows used native runtime cells. Post/reply layout misses, polls, rich text, and missing render data should be fixed in the native cell path rather than routed through a SwiftUI post-row fallback.

Status update (2026-05-31, runtime smoothness): the app now points `native/ios-app/project.yml` at an explicit `Configs/Fire-Info.plist` template that carries `CADisableMinimumFrameDurationOnPhone`, so iPhone builds opt into ProMotion instead of depending on generated-plist build settings. The topic-detail bottleneck was still main-thread budget rather than an explicit 60 fps cap. Poll-bearing original posts and replies stay on the native path: poll option text is prepared into lightweight render models, and poll controls are rendered by native runtime cells instead of falling back to SwiftUI `UIHostingController.sizeThatFits`.

Status update (2026-05-31, inline reply context): the nested-reply shortcut is now an inline expansion action, not a sheet-only affordance. Tapping "查看更多 ... 条回复" marks that root reply expanded and calls `FireTopicDetailStore.loadPostReplyContextIfNeeded`; fetched reply-context posts are merged back into the active Rust response-row render path before the topic render cache is rebuilt, so missing depth >= 2 rows become visible under their root reply. The shortcut shows a disabled loading title while the reply context request is in flight.

Status update (2026-05-31, no-op churn reduction): the topic-detail feed pipeline now receives a narrow runtime invalidation token and skips rebuilding the applied feed snapshot for unrelated store publications. Runtime content tokens no longer stringify reactions or polls with reflection; they use explicit compact signatures. Published background layouts are not applied with collection invalidation while the user is actively dragging or decelerating, and all native row layout keys use one inset-adjusted content-width source to avoid width-signature cache thrash on iPad and landscape devices.

Status update (2026-06-01, Texture migration and rich-text measurement): `FireTopicDetailFeedController` now uses Texture's `ASCollectionNode` rather than IGListKit for the topic-detail runtime. Original posts and replies bind to `FirePostCellNode`, where `ASTextNode` owns both rich-text layout and display; the old TextKit measurement fallback is no longer combined with the Texture height. Block-level cooked HTML rendering now normalizes only the next block boundary, so adjacent headings, paragraphs, code blocks, quotes, tables, dividers, and lists do not accumulate triple newlines. The shared Rust cooked HTML parser now exposes `<hr>` as a divider node through UniFFI, keeping protocol parsing and iOS rendering aligned. Scroll-driven visible-post reports are coalesced before crossing back into SwiftUI/store state.

Status update (2026-06-01, Texture interaction hardening): the Texture topic-detail runtime no longer relies on a SwiftUI root `DragGesture` to record reading interactions because that gesture competes with the embedded `ASCollectionNode` scroll recognizer. Scroll visibility changes now record activity after the collection view has actually moved. `FirePostCellNode` also constrains its content stack to the avatar-adjusted text width and only begins the swipe-to-reply pan for rightward horizontal movement, so vertical collection scrolling and `ASTextNode` wrapping keep priority. Markdown list items render through a hanging-indent paragraph style and unwrap the first `<p>` inside `<li>`, keeping the bullet marker and first item line together while wrapped continuation lines align under the item text.

Status update (2026-06-01, native chrome and paging visibility): topic-detail header/category chips and AI-summary chrome rows now use native Texture cell nodes (`FireTopicDetailHeaderCellNode` and `FireTopicDetailAISummaryCellNode`) instead of `UIHostingController` or `UIHostingConfiguration`. The SwiftUI boundary is limited to the outer `UIViewControllerRepresentable` host needed by the surrounding app navigation, while the detail scrolling surface, chrome rows, original post, replies, stats, topic vote, loading/error states, and reply footer are Texture/UIKit-owned. Topic-detail response pagination is triggered from Texture batch-fetch callbacks plus near-bottom scroll metrics, the loading footer is retained while continuous page requests are in flight, repeated load-more evaluation now keys off footer proximity as well as visible-tail position, background topic-screen refreshes retain already-appended reply rows instead of collapsing back to the first response page, and no-op item diffs no longer fall back to `reloadData`. Home feed paging uses page-token near-bottom detection, and ListKit section updates now defer only during refresh, allowing the append loading footer to become visible during normal bottom scrolling.

Status update (2026-06-02, refresh and response-session correctness): iOS topic-detail initial load, explicit force refresh, mutation refresh, and MessageBus-triggered refresh now use the Rust processed feed path (`loadTopicDetailFeed` / `refreshTopicDetailFeed`) before bridging back into `TopicScreenState`; Swift no longer has a separate raw `fetchTopicScreen` refresh path for these detail updates. Forced refresh bridging requires a ready feed snapshot, so `staleIfError` cache snapshots surface as refresh failures instead of silently reapplying stale body or reply data. Partial refreshes may retain already-appended reply rows to avoid collapsing the visible thread, but the response `nextCursor` always comes from the incoming Rust screen because response cursors are scoped to the Rust response session. Runtime invalidation also keys reply rows by Rust response-tree shape (`parentPostNumber`, depth, preorder, sibling metadata), and accepted load-more requests set `loadingMore` synchronously before the async page task starts.
...
- `native/ios-app/App/TopicDetail/Support/FireTopicPresentation.swift` `loadedWindowCount` / `missingPostIDs` (lines 248, 274) -- prefix-based hydration helpers that treat “loaded” as a contiguous stream head.
- `native/ios-app/App/TopicDetail/Feed/FireTopicDetailFeedController.swift` scroll callbacks plus `FireTopicDetailVisibilityCoordinator` / `FireTopicDetailPaginationCoordinator` -- visible-row, scroll-metric, and batch-fetch callbacks used to trigger response-page preload and reading-time timing.
- `native/ios-app/App/TopicDetail/Feed/FireTopicDetailVisibilityCoordinator.swift` `handlePendingScrollTargetIfNeeded()` plus `FireTopicDetailRuntimeConfiguration.scrollItem(for:)` -- scroll retry path; the request stays pending until the target runtime item exists in the Texture collection snapshot.
- `native/ios-app/App/TopicDetail/Feed/FireTopicDetailFeedController.swift`, `FireTopicDetailFeedModels.swift`, and `native/ios-app/App/ListKit/TopicDetail/FirePostCellNode.swift` -- native post/reply row rendering and viewport reporting for long threads.
