...
- 帖子详情链路：`FireTopicDetailView` -> `FireTopicDetailListHost` -> `FireTopicDetailListViewController` -> `FirePostCollectionViewCell`。
...
- 帖子详情页现在通过 `TopicDetailRuntime` 的 IGListKit/UICollectionView runtime 渲染。offscreen item 复用或重新配置时，`FirePostCollectionViewCell` 会重新绑定头像请求。
...
- `FireTopicDetailView` 顶层保留 SwiftUI shell，但正文阅读面已经切到 `FireTopicDetailListHost`。
- `FireTopicDetailListViewController` 为 post/reply item 绑定 `FirePostCollectionViewCell`。
- `FirePostCollectionViewCell` 直接通过 `FireRemoteImagePipeline` 加载头像，不再经过 `FirePostRow` / `FireAvatarView` fallback。
...
- 详情页：runtime collection cell 复用或重配后，`FirePostCollectionViewCell` 重新绑定头像请求。
...
- 首页 `FireTopicRow` 和详情 `FirePostCollectionViewCell` 自动受益。
