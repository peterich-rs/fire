...
## Phase 1 Spike 状态（2026-04-18）

- 已落地一个仅用于头像的 Fire 自有图片层：`FireAvatarImagePipeline` + `FireAvatarView` 内部状态保留。
- 这次没有引入第三方图片库，也没有扩到帖子正文图片、全屏 viewer 或保存能力。
- 当前实现的关键收益是：热头像在 view 重建时可以先同步命中 decoded image memory cache，再决定是否异步补请求，从而尽量避免回到字母占位头像。
