...
- `FireAvatarView` 在本地把 `avatar_template` 里的 `{size}` 替换成 `Int(size * UIScreen.main.scale)`，拼成绝对 URL 后直接调用 `AsyncImage(url:)`。
- 当前 iOS 工程里没有接入任何第三方图片库。仓库检索 `native/ios-app/` 没有发现 `Nuke`、`Kingfisher`、`SDWebImage` 等实际代码或依赖声明。现状就是系统原生 `AsyncImage`。
...
结论：iOS 应该尽快引入一个 platform-owned 的 Fire 自有图片包装层，底层优先选 `Nuke`，而不是继续直接把 `AsyncImage` 散落在各个业务 view 里。
...
- `FireAvatarView` 自己构造 URL，然后直接 `AsyncImage(url:)`。
...
所以首页和详情页虽然宿主不同，一个是 `UICollectionView`，一个是 `LazyVStack`，但头像加载实现是同一套：都落到 `FireAvatarView` + `AsyncImage`。

### 当前帖子图片链路

- `FireTopicPresentation.imageAttachments` 仍然是 Swift 侧能力，不在 Rust。
- `FireCookedImageCard` 和 `FireTopicImageViewer` 都在用 `AsyncImage`。
- `FireTopicImageViewer` 只是一个全屏 cover，不是完整 viewer：没有 pinch zoom、double tap、drag-to-dismiss 动画状态，也没有保存到相册。
...
这说明“图片上传”已经有清晰的 Rust-owned API 边界，而“图片显示”仍然没有统一平台管线。
...
### 方案 B：iOS 引入 Nuke，并包在 Fire 自己的图片层后面
...
- iOS 第一阶段引入 `Nuke`。
- 不让业务 view 直接散落使用第三方库，而是在平台侧建立 Fire 自己的图片入口。
...
- 新建平台图片管线封装。
- `FireAvatarView` 内部从 `AsyncImage` 切到统一图片层。
...
### Phase 2：扩展到帖子内容图片与 viewer

目标：把同一条图片管线扩到帖子正文图片。

范围：

- `FireCookedImageCard` 改走统一图片层。
- `FireTopicImageViewer` 改成完整 viewer：pinch zoom、double tap、drag-to-dismiss、进度/错误状态。
- 增加保存到 Photos 和系统分享。
...
2. 用一个最小 spike 验证 Nuke 在这两类宿主里是否能消除热图回闪。
   重点不是“它有没有缓存”，而是：
...
2. 现阶段最适合本项目的主方向是：平台侧建立 Fire 自有图片层，iOS 底座优先选 `Nuke`，Rust 保持在 metadata / upload / request descriptor 协作边界，不去做主图片栈。
