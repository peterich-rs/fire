...
- 帖子详情链路：`FireTopicDetailView` -> `FireTopicDetailListHost` -> `FireTopicDetailListViewController` -> `FirePostCellNode`。
...
- 帖子详情页现在通过 `TopicDetailRuntime` 的 Texture `ASCollectionNode` runtime 渲染。offscreen node 复用或重新配置时，`FirePostCellNode` 会重新绑定头像请求。
...
- `FireTopicDetailListViewController` 为 post/reply item 绑定 `FirePostCellNode`。
- `FirePostCellNode` 使用 Texture image nodes 加载头像和正文图片，不再经过 `FirePostRow` / `FireAvatarView` fallback。
...
- 详情页：runtime collection node 复用或重配后，`FirePostCellNode` 重新绑定头像请求。
...
- 首页 `FireTopicRow` 和详情 `FirePostCellNode` 自动受益。
