...
- 这次仍然没有引入第三方图片库；viewer 手势、保存/分享的最小切片已经落地，磁盘缓存、下采样、列表预取/取消和更广的媒体面统一还没开始。
...
- 帖子详情链路：`FireTopicDetailView` -> `FireTopicDetailCollectionView` -> `FirePostRow` -> `FireAvatarView`。
...
### 2. 为什么历史上 item 滚回屏幕内会先出现字母头像，再变成真实头像

根因更像是“视图重建 + 当时没有 app-owned 图片状态保留层”造成的，而不是当前自定义 `URLSession` 缓存策略导致的。
...
- 帖子详情页现在也通过 `FireCollectionHost` 包的 `UICollectionView` collection host 渲染。offscreen item 复用或重新配置时，`FirePostRow` / `FireAvatarView` 仍然会重建。

现在 `FireAvatarView` 已经走 Fire 自有 `FireRemoteImage` 层：请求键稳定后会先同步查询 decoded-image memory cache，命中时直接渲染真实图片；未命中时才显示 monogram 占位并异步加载。因此当前剩余问题不再是 `AsyncImage` 的 `.empty` 生命周期，而是这条 Fire 图片层还缺磁盘缓存、按目标尺寸下采样、集合预取/取消和更完整的内存压力策略。

### 3. 即使短期视图重建不可避免，理论上应该依赖什么缓存；为什么体感像没命中

当前实现已经有 Fire 自有 decoded-image memory cache 和 in-flight request coalescing；HTTP 缓存和上游 CDN 缓存仍然只负责减少字节回源，不能替代平台侧图片状态层。
...
1. Fire 内存缓存命中时可以绕过占位图。
2. 冷图、被 NSCache 驱逐的图、未预取的图仍会先显示 monogram。
3. 真实头像请求还要先走一次 `linux.do -> cdn.linux.do` 的重定向归一化入口，哪怕最终 CDN 图是热的，整个视图层仍然会重新启动一次异步加载流程。

因此，当前判断不再是“完全没命中缓存”，而是“Fire 已有热图内存命中能力，但还不是完整生产级图片栈”。

### 4. 现阶段最适合本项目架构的图片方案

结论：Phase 1 已经落地 platform-owned 的 Fire 自有头像图片包装层，把头像从直接依赖 `AsyncImage` 提升到“同步内存命中 + 异步补加载”的状态；是否在后续更大范围图片能力里引入 `Nuke`，留给磁盘缓存、下采样、预取和更多媒体 surface 统一时再判断。

### 5. 方案不能只管头像

当前仓库已经有比头像更完整的图片需求链路：

- 帖子内容图片：`FireTopicPresentation.imageAttachments(from:baseURLString:)` 现在复用 Rust `parseCookedHtml` 的 cooked-HTML AST 提取图片附件，再由 `FireCookedImageCard` / `FireTopicImageViewer` 用 Fire 自有远程图片层展示。
- 全屏查看：`FireTopicDetailView` 已经用 `fullScreenCover(item: $selectedImage)` 打开 `FireTopicImageViewer`；viewer 现在支持 pinch zoom、放大后拖拽平移、未放大时下拉关闭、系统分享和保存到相册，剩余双击缩放和更完整的媒体栈仍可继续补。
...
- 第一阶段已经解决头像热图回显的核心问题：不改首页/详情页宿主结构，只把头像切到 Fire 自有图片层。
- 第二阶段最小切片已经把同一条图片管线扩展到帖子内容图片与全屏查看器。
- 第三阶段继续推进磁盘缓存、下采样、预取/取消、更多媒体面和 Android 对齐；必要时再抽共享媒体模型。
...
- `FireTopicDetailView` 顶层保留 SwiftUI shell，但正文阅读面已经切到 `FireTopicDetailCollectionView`。
- `FireTopicDetailCollectionView` 为每个 topic-detail collection item 构造 `FirePostRow`。
- `FirePostRow` 里的头像仍然是 `FireAvatarView`。

所以首页和详情页虽然 screen shell 不同，但热路径 row 都已经跑在 collection-backed host 上；头像加载实现仍然是同一套：都落到 `FireAvatarView` + `FireAvatarImagePipeline`。

### 当前帖子图片链路

- `FireTopicPresentation.imageAttachments` 仍然是 iOS 侧的展示适配入口，但底层图片/链接提取已经来自 Rust `parseCookedHtml` 共享 AST。
- `FireCookedImageCard` 和 `FireTopicImageViewer` 现在已经改走同一条 Fire 自有远程图片层，不再直接依赖 `AsyncImage`。
- `FireTopicImageViewer` 现在已经支持 pinch zoom、放大后拖拽平移、未放大时下拉关闭，以及系统分享和保存到相册；double tap 仍未补。
...
历史头像滚动回显问题的第一根因是视图重建，第二根因是图片层没有 app-owned 的缓存/状态保留抽象。当前 Fire 自有图片层已经补上热图 decoded-memory 命中；剩余根因主要是冷图、驱逐图和未预取图没有磁盘缓存和列表预取兜底。

更具体地说：

- 首页：collection cell 复用导致 `UIHostingConfiguration` 重新配置内容，`FireTopicRow` / `FireAvatarView` 重建。
- 详情页：collection host 的 cell/item 复用或重配后，`FirePostRow` / `FireAvatarView` 重建。
- 头像 view 一旦重建，图片层必须能同步恢复已解码图片，否则仍会短暂回到占位。
- 当前 `FireRemoteImage` 已能处理热图同步恢复，但冷图、驱逐图和未预取图仍会显示 monogram。

因此，历史上用户看到的不是“加载失败”，而是“每次重建都先走占位图，再异步切回真实图”。当前这一路径已经改善，后续要继续补完整图片栈能力。
...
原因是当前仍然需要更完整的“图片状态层”，不是只靠“CDN 缓存头”：

- HTTP/CDN 缓存只能减少字节回源。
- 它不能让新的 view 实例自动跳过占位。
- 它也不能在 Fire 内存缓存被驱逐后，帮应用从磁盘缓存或预取状态同步回填。

换句话说，当前体验已经从“缓存存在，但 UI 不知道怎么无闪回地消费它”推进到“热图能无闪回消费，冷图和被驱逐图还需要完整图片栈兜底”。

## 方案比较

### 方案 A：继续直接使用 `AsyncImage`
...
结论：这个历史基线已经不再是头像/帖子正文图片的当前实现，也不适合重新作为长期方案。
...
结论：对磁盘缓存、下采样、预取和更多媒体 surface 来说依然是强候选，但不是当前已落地头像/帖子图片最小切片的必要前提。
...
- iOS 第一阶段已经落地 Fire 自有头像图片层，底座是 `URLSession` + decoded image memory cache + in-flight request coalescing。
- 不让业务 view 直接散落图片实现细节，而是在平台侧建立 Fire 自己的图片入口。
- 第一阶段已替换 `FireAvatarView` 的内部实现，首页和帖子详情自动受益。
- 帖子图片、viewer、保存等需求已经进入同一条管线；后续是否把底座升级到 `Nuke` 这类更完整实现，取决于磁盘缓存、下采样、预取和动画格式需求。
...
- 已完成：新建平台图片管线封装。
- 已完成：`FireAvatarView` 内部从直接依赖 `AsyncImage` 切到统一图片层。
- 已完成：首页 `FireTopicRow` 和详情 `FirePostRow` 自动受益。
- 顺带覆盖通知、关注列表、个人资料页头像可以接受，但不是本阶段验收重点。

明确不做：

- 不改首页 `FireCollectionHost` 结构。
- 不在这个图片阶段继续改帖子详情的 ListKit/collection host 宿主结构。
- 当时不重写 viewer。
- 当时不迁移帖子内容图片。
...
- 未完成：double tap。
- 已完成：保存到 Photos 和系统分享，iOS 端按 add-only 照片权限申请。

这一步已完成解析边界收敛：iOS 的 `imageAttachments(from:baseURLString:)` 和 Android 的 `FireCookedHtmlRenderer` 都消费 Rust `parseCookedHtml` 产出的 cooked-HTML AST；平台侧继续负责图片请求、解码、缓存、viewer 手势、保存和分享。
...
2. 用一个最小 spike 验证 Fire 自有图片层在这两类宿主里是否能消除热图回闪。
   ...
   - 在 topic detail collection row / cell 重建时是否能直接从内存命中恢复。
...
- iOS 保存到 Photos 的 add-only 授权、拒绝授权和保存失败提示路径。
...
1. 当前头像回显的关键不是“完全没缓存”，而是首页/详情页宿主都会触发 view 重建；Fire 自有图片层已经让热图可以同步命中，剩余生产化缺口在磁盘缓存、下采样、预取/取消和更完整的内存压力策略。
2. 现阶段最适合本项目的主方向仍是：平台侧建立 Fire 自有图片层，Rust 保持在 metadata / upload / request descriptor 协作边界，不去做主图片栈；是否引入 `Nuke` 留给后续更大范围图片面统一时再判断。
3. 第一阶段头像和第二阶段帖子图片/viewer 最小切片已经落地；后续继续推进更多媒体 surface、磁盘缓存、预取和 Android 对齐，不要把所有 media 问题捆成一次大改。
