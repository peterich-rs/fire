...
Real screenshots, preview videos, store records, legal review, device performance numbers, and accessibility audit results are not generated by these docs. Record final evidence in `release-gate-evidence.md`.

Before shipping, run the full release-readiness wrapper:

```bash
scripts/verify-release-readiness.sh
```

It runs the shared release evidence suite. It is expected to fail until all
manual release evidence is complete.
...
accepted-waiver notes are rejected.
