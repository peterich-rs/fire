...
- `scripts/test-release-verifiers.sh` is expected to pass without real manual evidence because it uses isolated temporary fixtures to regression-test the release verifiers themselves, including release-readiness wrapper wiring, fake-evidence marker rejection, malformed store-media failures, non-measurement performance results, target misses marked `Pass`, dead local evidence paths, weak accepted-waiver notes, and weak internal-testing/privacy metadata.
