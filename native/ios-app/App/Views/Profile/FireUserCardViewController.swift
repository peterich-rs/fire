...
final class FireUserCardViewController: UIViewController {
    ...
    private func configureLayout() {
        ...
        handleLabel.font = .systemFont(ofSize: 14, weight: .regular)
    ...
    private func makeActions() -> UIStackView {
        let row = UIStackView()
        row.axis = .vertical
        row.spacing = 8
        row.addArrangedSubview(makeButton(title: "查看主页", filled: false, action: #selector(openProfile)))
        row.addArrangedSubview(makeButton(title: "发私信", filled: false, action: #selector(openPrivateMessage)))
        row.addArrangedSubview(makeButton(title: "聊天", filled: true, action: #selector(openChat)))
        return row
    }
