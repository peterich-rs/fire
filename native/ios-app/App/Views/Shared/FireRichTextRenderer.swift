...
enum FireRichTextParser {
    ...
    /// Rust now owns the semantic rich-text transform; iOS only adapts the
    /// shared render document to the existing native text/image renderer model.
    static func parse(html: String, baseURLString: String) -> FireRichTextContent {
        ...
        return parse(document: renderCookedHtml(rawHtml: html, baseUrl: baseURLString))
    }

    static func parse(post: TopicPostState, baseURLString: String) -> FireRichTextContent {
        parse(document: post.renderDocument ?? renderCookedHtml(rawHtml: post.cooked, baseUrl: baseURLString))
    }

    static func parse(document: RenderDocumentState?) -> FireRichTextContent {
        ...
        let nodes = tree.root.map {
            mapRenderedChildren(of: $0, tree: tree)
        } ?? []
        return FireRichTextContent(
            nodes: nodes,
            plainText: document.plainText,
    ...
    private static func mapRenderedChildren(
        of block: RenderBlockState,
        tree: RenderBlockTree
    ) -> [FireRichTextNode] {
        tree.children(of: block).flatMap { child in
            mapRenderBlock(child, tree: tree)
        }
    }

    private static func mapRenderBlock(
        _ block: RenderBlockState,
        tree: RenderBlockTree
    ) -> [FireRichTextNode] {
        let children = mapRenderedChildren(of: block, tree: tree)
        ...
        case .list(let ordered):
            let items = tree.children(of: block).compactMap { child -> [FireRichTextNode]? in
                ...
                guard case let .some(.listItem(itemChildren)) = mapRenderBlock(child, tree: tree).first else {
                    return nil
                }
                return itemChildren
            }
