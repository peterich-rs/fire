...
- Swift Package dependencies are declared in `project.yml`: IGListKit 5.2.0, Nuke 12.8.0, CrashReporter, ConfettiSwiftUI, and the local `TextureCore` package.
- `TextureCore` wraps `LocalPackages/TextureCore/Artifacts/AsyncDisplayKit.xcframework`; regenerate it with `native/ios-app/scripts/build_texture_xcframework.sh` when Texture 3.2.0 needs to be rebuilt.
...
- Topic detail now renders its reading surface through a dedicated `TopicDetailRuntime` UIKit collection controller backed by IGListKit while keeping the SwiftUI shell for navigation, sheets, and the persistent quick-reply bar; it still hides the tab bar as a dedicated reading page and promotes the original post into the topic header as the main body.
...
- Topic-detail cooked text and post images now render through Texture nodes inside native cells: `FirePostRichTextContainerView` uses `ASTextNode`, and post attachments use `ASImageNode` placeholders. Texture is consumed as the local `TextureCore` binary Swift package, not through CocoaPods.
...
- Topic detail cursor pagination now appends reply rows and cooked-content caches incrementally on a background task, coalesces detail/render/loading-footer updates into one list revision, races slow topic-detail operations against a real timeout, and keeps diff tokens lightweight so large cooked HTML bodies are not copied into every collection revision.
