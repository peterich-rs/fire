...
- The homepage `latest` feed now coalesces MessageBus topic-list updates on iOS, enforces a 30-second minimum refresh interval, and batches `/latest.json?topic_ids=...` incremental reloads while the active view is the unfiltered latest list. Events that cannot be represented as topic-id incremental updates are ignored instead of forcing a full `/latest.json` refresh in the background.
- Topic detail now renders its reading surface through a dedicated `TopicDetailRuntime` controller backed by Texture `ASCollectionNode` while keeping the SwiftUI shell for navigation, sheets, and the persistent quick-reply bar; it still hides the tab bar as a dedicated reading page and promotes the original post into the topic header as the main body. The controller skips no-op snapshot updates, coalesces scroll-driven visible-post reports, and force-publishes visibility after collection updates complete. Header/category chips and AI summary remain SwiftUI-hosted screen chrome, while stats, topic vote, body-state, replies header, reply-footer, notice, original post, and reply rows use native Texture/runtime cells. The old `FireTopicDetailCollectionView`/`FirePostRow` SwiftUI post-row fallback has been removed.
- Topic detail initial load now enters Rust through `loadTopicDetailFeed`, which stores processed feed snapshots in the Rust SQLite `fire-store` cache before iOS bridges the snapshot back into the current `TopicScreenState` render path. Reply pagination still uses shared Rust `fetchTopicResponsePage`; the shared cursor is bounded by both root count and row count, so a single large reply branch continues through `nextBranchOffset` instead of blocking the iOS list on a huge one-page payload. The Texture runtime triggers the next response page from rendered-tail visibility and scroll-distance metrics instead of max `postNumber`, keeps an explicit tappable load-more footer while more reply pages are available, and filters pure overlap rows before applying incremental append snapshots.
