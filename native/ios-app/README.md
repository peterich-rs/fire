...
- `App/FireTopicDetailView.swift`
  ...
  - reports visible-post reading timings through a native viewport tracker that flushes into the shared Rust `/topics/timings` API on a conservative 60-second cadence with host-side retry backoff
...
- `App/ListKit/`
  ...
  - topic detail now bypasses the generic diffable ListKit host and uses `App/TopicDetailRuntime/FireTopicDetailListViewController.swift`, where Texture `ASCollectionNode` owns the collection runtime, range lifecycle, display callbacks, and batch-fetch callbacks
...
7. After login or restore, render the topic feeds through `fetchTopicList`, render iOS topic detail through the processed feed APIs (`loadTopicDetailFeed` / `refreshTopicDetailFeed`), and let topic detail fetch optional AI summaries through `fetchTopicAiSummary`.
...
- Topic detail now renders its reading surface through a dedicated `TopicDetailRuntime` controller backed by Texture `ASCollectionNode` while keeping the SwiftUI shell for navigation, sheets, and the persistent quick-reply bar; it still hides the tab bar as a dedicated reading page. The controller skips no-op snapshot updates, coalesces scroll-driven visible-post reports, force-publishes visibility after collection updates complete, and keeps already-appended reply rows stable across partial topic-screen refreshes. Header/category chips, AI summary, stats, topic vote, body-state, replies header, reply-footer, notice, original post, and reply rows all use native Texture/runtime cells. The old `FireTopicDetailCollectionView`/`FirePostRow` SwiftUI post-row fallback has been removed.
- Topic detail initial load, explicit force refresh, mutation refresh, and MessageBus-triggered refresh now enter Rust through `loadTopicDetailFeed` / `refreshTopicDetailFeed`, which store processed feed snapshots in the Rust SQLite `fire-store` cache before iOS bridges the snapshot back into the current `TopicScreenState` render path. Forced refreshes only apply bridge snapshots whose load state is ready, so a stale-if-error cache snapshot cannot masquerade as a refreshed body. Reply pagination still uses shared Rust `fetchTopicResponsePage`; the shared cursor is bounded by both root count and row count, is scoped to the current response session, and must be replaced by the incoming cursor after a fresh screen refresh even when iOS preserves already-appended reply rows for visual stability. The Texture runtime triggers the next response page from rendered-tail visibility and scroll-distance metrics instead of max `postNumber`, keeps an explicit tappable load-more footer while more reply pages are available, sets the loading-more state synchronously when the store accepts a page request, retries the footer handoff only after a rejected request, relayouts visible post rows in place for expand-more interactions, and filters pure overlap rows before applying incremental append snapshots.
