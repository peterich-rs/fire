...
- `App/Stores/FireTopicDetailStore.swift`
  ...
  - renders Rust-owned `TopicPostBoostState` short replies as a native Texture body overlay/barrage for original posts with visible body text, and as a fixed two-row ticker for replies or posts without a body text target; the cell consumes Rust-derived `displayText`, caps visible overlay boosts to a normalized once-per-batch five-lane barrage with pause/resume during scroll, keeps only fixed ticker height/signature in `FirePostCellLayoutKey` instead of parsing Boost `cooked` in Swift, and only runs Boost animations on visible cells while the detail feed is idle
