...
- Topic detail now pages replies through shared Rust `fetchTopicScreen` / `fetchTopicResponsePage` calls, with the original post rendered separately and reply rows arriving in Rust-owned tree order rather than host-rebuilt window order. The shared cursor is bounded by both root count and row count, so a single large reply branch continues through `nextBranchOffset` instead of blocking the iOS list on a huge one-page payload.
