...
- Topic detail now supports per-post reply targeting, quote reply insertion from Rust-provided `RenderDocumentState.plainText`, post likes, a searchable Rust-bootstrap-backed reaction picker with reaction-user lookup, post-level bookmarks, edit/delete/recover, and flag reporting through the shared Rust write APIs; the flag sheet uses server-provided post action types when available and falls back to the common LinuxDo / Discourse flag types only if that metadata is unavailable.
