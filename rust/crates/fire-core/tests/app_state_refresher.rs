...
async fn refresh_all_runs_core_immediately_and_secondary_after_delay() {
    ...
    assert!(requests[1].contains("GET /u/alice/summary.json HTTP/1.1"));
    assert!(requests[2].contains("GET /u/alice/bookmarks.json HTTP/1.1"));
    assert!(requests[3].contains("GET /read.json HTTP/1.1"));
    assert!(
        requests[4]
